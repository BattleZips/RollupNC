...
/// @notice copied from zk-kit because missing update
library IncrementalBinaryTree {
    ...
    /// @dev Update a leaf in a tree
    /// @param self: Tree data
    /// @param leaf: Leaf to insert as update
    /// @param index: Index of leaf to replace
    function update(IncrementalTreeData storage self, uint256 leaf, uint256 index) public {
        require(
            leaf < SNARK_SCALAR_FIELD,
            "IncrementalBinaryTree: leaf must be < SNARK_SCALAR_FIELD"
        );
        require(
            index < self.numberOfLeaves,
            "IncrementalBinaryTree: leaf does not exist"
        );
...
/* Each incremental tree has certain properties and data that will be used to add new leaves. */
